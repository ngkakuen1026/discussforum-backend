...
describe('Post vote routes - integration tests (supertest) with mocked DB', () => {
  ...
  it('GET /api/v1/posts/votes/:postId -> 404 when post not found', async () => {
  ...
  it('GET /api/v1/posts/votes/:postId -> 200 returns grouped votes when post exists', async () => {
    pool.query
  ...
  it('POST /api/v1/posts/votes/:postId -> 200 records vote (authenticated) and notifies post owner', async () => {
    pool.query
    ...
    expect(notificationUtils.createNotification).toHaveBeenCalledWith(99, expect.stringContaining('liked your post'), 'like', 10);
  });

  it('POST /api/v1/posts/votes/:postId -> 403 when user already voted', async () => {
    pool.query
  ...
  it('POST /api/v1/posts/votes/:postId -> 400 when invalid voteType provided', async () => {
    const res = await request(app)
      .post('/api/v1/posts/votes/10')
      .set('x-test-user', 'test-user')
